...
class MoE(nn.Module):
    ...
    def __init__(self, n_tasks, input_size, output_size, num_experts, hidden_size, k=4):
        ...
        self.experts = nn.ModuleList(
            [MLPExpert(self.input_size, self.output_size, self.hidden_size) for i in range(self.num_experts)]
        )
        self.w_gate = nn.ParameterList(
            [nn.Parameter(torch.zeros(input_size, num_experts), requires_grad=True) for i in range(n_tasks)]
        )
    ...
    def init_tasks_probs(self):
        self.tasks_probs = []
        for _j in range(self.n_tasks):
            temp = []
            for _i in range(self.num_experts):
                temp.append([])
            self.tasks_probs.append(temp)

    def display_tasks_probs(self):
        print(
            f"\nProbability distribution on experts for each task, computed over {len(self.tasks_probs[0][0])} data points:"
        )
        for j in range(self.n_tasks):
            probs = self.tasks_probs[j]
            probs = np.array([np.mean(x) for x in probs])
            prob_std = np.std(probs)
            probs = [f"{x:.4f}" for x in probs]
            print(f"Task {j + 1} / {self.n_tasks}, distribution across experts: {probs}, std: {prob_std:.4f}")
        self.init_tasks_probs()
