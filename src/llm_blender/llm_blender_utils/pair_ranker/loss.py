...
def permutation_prob(scores, level=1):
    ...
    if level > 1:
        for i in range(n_candidates):
            ...
            next_prob = permutation_prob(
                scores_except_i, level=level - 1
            )  # [batch_size, (n_candidates-1)*(n_candidates-2)*...(n_candidates-level)]
...
def ListNet_loss(pred_scores, scores, top_k_permutation=1):
    ...
    """
    # apply exp
    exp_pred_scores = torch.exp(
        pred_scores - torch.max(pred_scores, dim=1, keepdim=True)[0]
...
def infoNCE_loss(sim_mat, labels, temperature=0.07):
    ...
    torch.sum(torch.exp(pos_sim), dim=1)
...
def simcls_loss(sim_mat, target_sim, scores):
    ...
    gold_margin_loss = loss_func(
        target_sim.repeat(sim_mat.shape[1], 1).transpose(0, 1), sim_mat, torch.ones_like(sim_mat)
    )
