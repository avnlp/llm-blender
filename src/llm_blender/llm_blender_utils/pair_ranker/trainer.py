...
def compute_metrics_for_scr(eval_pred: EvalPrediction) -> Dict[str, float]:
    ...
    pred_ranks[np.arange(pred_sort_indices.shape[0])[:, None], pred_sort_indices] = np.arange(
        pred_sort_indices.shape[-1]
    )
    ...
    for i in range(sel_scores.shape[-1]):
        metrics["sel"][f"metric{i + 1}"] = np.mean(sel_scores[:, i])
        metrics["oracle"][f"metric{i + 1}"] = np.mean(oracle_sel_scores[:, i])
    return metrics
...
def compute_metrics_for_pairranker(eval_pred: EvalPrediction) -> Dict[str, float]:
    ...
    if logits.shape[1] == 3:
        ...
        msg = f"Invalid logits shape: {logits.shape}"
    ...
    for i in range(n_tasks):
        metrics["sel"][f"metric_{i + 1}"] = np.mean(pred_best_scores[:, i])
        metrics["oracle"][f"metric_{i + 1}"] = np.mean(oracle_best_scores[:, i])
        metrics["top_beam"][f"metric_{i + 1}"] = np.mean(scores[:, 0, i])
        metrics["gain"][f"metric_{i + 1}"] = (
            metrics["sel"][f"metric_{i + 1}"] / metrics["top_beam"][f"metric_{i + 1}"] - 1
        )
