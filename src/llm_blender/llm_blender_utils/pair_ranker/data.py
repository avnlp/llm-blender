...
from llm_blender.llm_blender_utils.common.evaluation import METRIC_WEIGHTS
...
def load_data(data_path, args, max_size=None):
    ...
    for item in data:
        ...
        if args.candidate_decoding_methods is not None:
            candidates = [
                candidate for candidate in candidates if candidate["decoding_method"] in args.candidate_decoding_methods
            ]
        if len(candidates) == 0:
            available_model_methods = {
                (candidate["model"], candidate["decoding_method"]) for candidate in item["candidates"]
            }
            msg = "No candidates left after filtering, available models and methods are: \n{}".format(
                "\n".join([str(x) for x in available_model_methods])
            )
            raise ValueError(msg)
        item["candidates"] = candidates
        for candidate in item["candidates"]:
            candidate["scores"] = {metric: candidate["scores"][metric] for metric in args.metrics}
...
def check_and_normalize_scores(examples):
    ...
    candidate_scores = {
        task: [np.mean([ex["candidates"][i]["scores"][task] for ex in examples]) for i in range(n_candidates)]
        for task in task_names
    }
    for task in task_names:
