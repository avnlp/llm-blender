...
from llm_blender.llm_blender_utils.pair_ranker.layers import MoE
from llm_blender.llm_blender_utils.pair_ranker.loss import simcls_loss
...
class SummaReranker(nn.Module):
    ...
    def __init__(self, pretrained_model, args, tokenizer=None):
        ...
        # MoE
        self.moe = MoE(
            self.n_tasks, self.hidden_size, self.hidden_size, 2 * self.n_tasks, self.hidden_size, k=self.n_tasks
        )
    ...
    def _forawrd(self, input_ids, attention_mask):
        ...
        outputs = self.pretrained_model(
            input_ids=to_model_input_ids, attention_mask=to_model_attention_mask, output_hidden_states=True
        )
        ...
        pred_scores = torch.cat([tower(moe_pred) for moe_pred, tower in zip(moe_preds, self.towers)], dim=-1)
    ...
    def forward(self, input_ids, attention_mask, scores=None):
        ...
        pred_scores = pred_scores.reshape(batch_size, -1, n_candidates).transpose(
            1, 2
        )  # [batch_size, n_candidates, n_tasks]
...
class DualReranker(nn.Module):
    ...
    def _forward(
        ...
        source_encs = self.source_encoder(
            input_ids=source_ids, attention_mask=source_attention_mask, output_hidden_states=True
        )["last_hidden_state"][:, 0, :]
        source_encs = F.normalize(source_encs, dim=-1)

        candidate_encs = self.candidate_encoder(
            input_ids=candidate_ids, attention_mask=candidate_attention_mask, output_hidden_states=True
        )["last_hidden_state"][:, 0, :].reshape(
        ...
        target_encs = self.candidate_encoder(
            input_ids=target_ids, attention_mask=target_attention_mask, output_hidden_states=True
        )["last_hidden_state"][:, 0, :].reshape(batch_size, 1, -1)
        target_encs = F.normalize(target_encs, dim=-1)
        sim_mat = torch.matmul(source_encs.unsqueeze(1), candidate_encs.transpose(1, 2)).squeeze(
            1
    ...
    def forward(
        ...
        if scores is not None:
            labels = (
                torch.eq(torch.sum(scores, dim=-1), torch.max(torch.sum(scores, dim=-1), dim=1, keepdim=True)[0])
                .float()
            ...
            if self.training:
                ...
                candidate_attention_mask = candidate_attention_mask[
                    torch.arange(batch_size).unsqueeze(-1), selected_idx
                ]
                scores = scores[torch.arange(batch_size).unsqueeze(-1), selected_idx]
                labels = labels[torch.arange(batch_size).unsqueeze(-1), selected_idx]
...
class CrossCompareReranker(nn.Module):
    ...
    def sampling(self, candidate_ids, candidate_attention_mask, scores):
        """
        Args:
        ...
        if self.sub_sampling_mode == "top_bottom":
            ...
            msg = f"Unknown sampling mode: {self.sub_sampling_mode}"
            raise ValueError(msg)
    ...
    def forward(
        ...
        if self.training:
            ...
            for i in range(batch_size):
                ...
                extended_source_attention_mask.append(
                    source_attention_mask[i].unsqueeze(0).repeat(sampling_results["cand1_ids"].shape[0], 1)
                )
...
def sub_sampling(mode, num_pos, num_neg, ratio, scores):
    ...
    if mode == "uniform":
        ...
        selected_idx = torch.stack(
            [torch.randperm(n_candidates)[: int(n_candidates * ratio)] for _ in range(batch_size)], dim=0
        )  # [batch_size, n_candidates * ratio]
