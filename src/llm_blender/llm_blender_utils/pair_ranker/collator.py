...
class CrossCompareCollator:
    def __init__(
        ...
        tokenizer.add_tokens(
            [self.source_prefix, self.candidate1_prefix, self.candidate2_prefix, self.candidate_prefix]
        )  # debug
    ...
    def __call__(self, batch):
        ...
        if self.mannually_add_sep_token:
            ...
            batch_candidates = [
                get_truncated_text(c, self.tokenizer, self.candidate_maxlength + remaining_length // 2)
            ...
            candidate_ids, candidate_masks = encode_batch_text(
                batch_candidates, self.tokenizer, self.candidate_maxlength + remaining_length // 2
            )
            cand_valid_positions = candidate_masks.reshape(-1, candidate_masks.shape[-1]).any(dim=0)
            candidate_ids = candidate_ids[:, :, cand_valid_positions]
            candidate_masks = candidate_masks[:, :, cand_valid_positions]
